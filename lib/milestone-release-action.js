    ...
    return new Promise((resolve, reject) => {
        const req = httpsModule.request({
            ...
            headers: {
                ...
                'Authorization': isGitHubDotCom(serverUrl) ? ['Bearer', token].join(' ') : ['token', token].join(' '),
...
async function run({
    ...
    try {
        ...
        if (milestoneNext != null && milestoneNext.length > 0) {
            const nextMilestoneTitle = milestoneNext.replace('-SNAPSHOT', '');
            const nextMilestone = milestones.find((currentMilestone) => currentMilestone.title === nextMilestoneTitle);

            if (nextMilestone == null) {
                await createMilestone(octokit, ghOwner, ghRepo, nextMilestoneTitle);
                logger.log(`Created Milestone ${nextMilestoneTitle}`);
            }
        }
