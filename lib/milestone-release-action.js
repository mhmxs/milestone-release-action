...
async function run({
    ...
    try {
        ...
        if (milestoneNext != null && milestoneNext.length > 0) {
            const milestoneTitleCreate = milestoneNext.replace('-SNAPSHOT', '');
            const nextMilestone = milestones.find((currentMilestone) => currentMilestone.title === milestoneTitleCreate);

            if (nextMilestone == null) {
                await createMilestone(octokit, ghOwner, ghRepo, milestoneTitleCreate);
                logger.log(`Created Milestone ${milestoneTitleCreate}`);
            }
        }
